    ...
    if predict_button:
        ...
        try:
            ...
            try:
                ...
                # 获取最重要的10个特征
                shap_values_no_id = shap_value[1:]  # 跳过ID列
                feature_impacts = []

                for i, feature in enumerate(feature_names_display):
                    impact_value = float(shap_values_no_id[i])
                    feature_name = feature_dict.get(feature, feature)
                    feature_impacts.append({
                        'name': feature_name,
                        'value': impact_value,
                        'abs_value': abs(impact_value)
                    })
                ...
                <head>
                    ...
                    <style>
                        ...
                        .positive-color {{ background: #ff6b6b; }}
                        .negative-color {{ background: #4ecdc4; }}
                    </style>
                </head>
                <body>
                    <div class="waterfall-container">
                        <div class="waterfall-title">特征对累积活产预测的影响</div>
                        <div class="base-value">基础预测值: {expected_value:.3f}</div>
                """

                # 计算最大绝对值用于标准化
                max_abs_value = max([f['abs_value'] for f in top_features])
                ...
                for feature in top_features:
                    ...
                    if value >= 0:
                        bar_class = "bar-positive"
                        sign = "+"
                    else:
                        bar_class = "bar-negative"
                        sign = ""
                    ...
                    html_content += f"""
                        <div class="feature-row">
                            ...
                            <div class="bar-container">
                                <div class="{bar_class}" style="width: {width}%;">
                                    <div class="value-label">{sign}{value:.3f}</div>
                                </div>
                            </div>
                        </div>
                    """
                ...
                html_content += f"""
                        <div class="legend">
                            <div class="legend-item">
                                ...
                                <span>正向影响（增加活产概率）</span>
                            </div>
                            <div class="legend-item">
                                <div class="legend-color negative-color"></div>
                                <span>负向影响（降低活产概率）</span>
                            </div>
                        </div>
                        <div class="base-value">最终预测值: {expected_value + sum([f['value'] for f in top_features]):.3f}</div>
                    </div>
                </body>
                </html>
                """
                ...
                st.success("✅ HTML瀑布图显示成功，支持中文字体")

            except Exception as html_error:
                st.error(f"HTML瀑布图生成失败: {html_error}")

            # 备用：matplotlib瀑布图
            st.write("### 📊 备用瀑布图（matplotlib版本）")
            try:
                # 创建SHAP瀑布图
                import matplotlib.font_manager as fm
